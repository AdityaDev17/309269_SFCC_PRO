...
const Cart = () => {
	...
	useEffect(() => {
		if (data?.couponItems && data?.couponItems[0]?.statusCode === "applied") {
			...
			const matchedAdjustment = data.orderPriceAdjustments?.find(
				(item: { couponCode: string }) => item.couponCode === name,
			);
	...
	const removeCoupon = useMutation({
		mutationFn: () =>
			graphqlRequest(REMOVE_COUPON, {
				input: { basketId, couponId: discountCode?.couponId },
			}),
		onSuccess: () => {
			refetch();
		},
					...
					<>
						...
						<div className={styles.orderSummarySection}>
							...
							<OrderSummary
								...
								errorMsg={
									Number.parseInt(data?.productTotal ?? "0") > 50
										? ""
