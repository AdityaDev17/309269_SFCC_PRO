import { ADD_ITEM_TO_BASKET, CREATE_CART, DELETE_BASKET_ITEM, GET_BASKET, UPDATE_BASKET_ITEM } from "@/common/schema";
...
const prepareCartItems = (response: CartItemResponse[], currency: string) => {
  cartItems = response?.map((item: any) => ({
    id: item?.productId,
    name: item?.itemText,
    description: "",
...
export const handleUpdateQuantity = async (itemId: string, newQuantity: number) => {
  console.log("id", itemId, newQuantity);
  const basketId = sessionStorage.getItem("basketId") ?? "";
  try {
    const response = await graphqlRequest(UPDATE_BASKET_ITEM, {input:{
      basketId,
      itemId,
...
export const addToBasket = async (productId:string) => {
    const basketId = sessionStorage.getItem("basketId");
    if (basketId) {
        const response = await graphqlRequest(ADD_ITEM_TO_BASKET, {input:{
            basketId,
        ...
        const response = await graphqlRequest(CREATE_CART, {input:{
            items: [{ productId, quantity: 1 }],
        }});
        prepareCartItems(
